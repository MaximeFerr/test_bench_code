#UNDER DEVELOPMENT!
# Please use v1.4 untill final  setup!!!!
# For windows > linux version still under dev
# -------------------------------------------
...
delay3 = 0.4 #delay in seconds for cooldown during phase shift sweep
...
PhaseFinal = 60 # Final Phase of phase sweep in °
PhaseStep = 30 # Phase step value during sweep in °
...
Sequence = '3' #400 ## At this moment, this is just a number value. No effect on scope
delayOscillo = 0.5
...
def GetBuffer(dmm, wait_meas_complete: bool = True) -> list[float]:
    ...
    if wait_meas_complete:
        ...
        buffer_str = dmm.query('R?', delay=2)
        time.sleep(2)
...
def ConfigForDCMeasure(dmm, measure_type: str = 'VOLT', **config):
    ...
    if 'trig_delay' in config:
        ...
        cmd_buf.append('TRIG:DEL:AUTO 1')
    #cmd_buf.append('TRIG:SLOP:POS')
    cmd_buf.append('TRIG:SLOP:NEG')
    SendCmd(dmm, cmd_buf, delay=0.05)
...
def DCSweepVoltageSource(Board):
    ...
    print('Source has been ramped up OK')
    time.sleep(delay)
    ...
    time.sleep(delay)
    print('sweep phase has been done OK')
    time.sleep(delay)
...
def main():
    ...
    ##      print(f"Sent command with frequency: {frequency_value}, message: {message1}")
        ...
        ConfigForDCMeasure(DMM3065_current, measure_type='current', range=0.2, trig_source='EXT',
                           trig_count=1, trig_delay='AUTO', nplc=1, sample_count=1)
        ConfigForDCMeasure(DMM3065_voltage,  measure_type='voltage', range=200, trig_source='EXT',
                           trig_count=1, trig_delay='AUTO', nplc=1, sample_count=1)
